...
DEFINE num_bins 3600          # 1 hour total experiment time
...
ACTION MAIN
    ...
    VIDEO(99999999999, "distance_traveled_tracking")
...
ACTION SMALL_SAMPLE
    ...
    LOGDATA(DATA_SNAPSHOT, "small_begin")
    WAIT(bin_length)
    LOGDATA(DATA_SNAPSHOT, "small_end")

    LOGDATA(DATA_SELECT, "small_begin")
    LOGDATA(DATA_DELTA, "small_end")

    SET(LOG_STREAM, 0)
    LOGCREATE("RUNTIME|TEMPERATURE1|COUNTER2|TEXT:DISTANCE")
    LOGAPPEND("ZONE_DISTANCES:A* Z*")
    LOGRUN()
    LOGCREATE("RUNTIME|TEMPERATURE1|COUNTER2|TEXT:TIME")
    LOGAPPEND("ZONE_TIMERS:A* Z*")
    LOGRUN()
